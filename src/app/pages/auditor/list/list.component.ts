  ...
  ngOnInit(): void {
    ...
    */
    this.breadCrumbItems = [
      { label: 'Auditorias' },
      { label: 'Mis Auditorias', active: true }
    ];
    ...
    if (localStorage.getItem('toast')) {
      this.toastService.show('Auditoria Creada.', { classname: 'bg-success text-center text-white', delay: 5000 });
      ...
      this.projectsService.get().pipe().subscribe(
        (data: any) => {
          ...
          for (var j = 0; j < resp.length; j++) {
            if(resp[j].tipoProyectoId && resp[j].tipoProyectoId != '' && resp[j].tipoProyectoId != null && resp[j].tipoProyectoId == 7){
