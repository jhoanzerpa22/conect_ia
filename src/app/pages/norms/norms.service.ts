...
import {SortColumn, SortDirection} from './norms-sortable.directive';

interface SearchResult {
  bodylegal: NormsModel[];
  total: number;
}
...
function sort(bodylegal: NormsModel[], column: SortColumn, direction: string): NormsModel[] {
  if (direction === '' || column === '') {
    return bodylegal;
  } else {
...
function matches(installation: NormsModel, term: string, pipe: PipeTransform) {
...
export class NormsService {
  ...
  private _bodylegal$ = new BehaviorSubject<NormsModel[]>([]);
  ...
  private _search(): Observable<SearchResult> {
    ...
    let data: any = this.bodylegal_data.length > 0 ? this.bodylegal_data : Norms;
